...
def compute_exclusion_bisection(metric_config: Dict[str,Any],
    ...
    if deformation == "mean":
        ...
        if verbose:
            print(f"=============== {metric_config['name']} - only diag of cov matrix ===============")
    elif deformation == "off":
        if verbose:
            print(f"=============== {metric_config['name']} - only off-diag of cov matrix ===============")
    elif deformation == "cov":
        if verbose:
            print(f"=============== {metric_config['name']} - only cov matrix ===============")
    elif deformation == "mean_std":
        if verbose:
            print(f"=============== {metric_config['name']} - mean + diag of cov matrix ===============")
    elif deformation == "mean_off":
        if verbose:
            print(f"=============== {metric_config['name']} - mean + off-diag of cov matrix ===============")
    elif deformation == "mean_cov":
        if verbose:
            print(f"=============== {metric_config['name']} - mean + cov matrix ===============")
    else:
    ...
    dist_1 = MixtureDistributions.MultiNormalFromMixtureGaussian(ncomp = ncomp,
                                                                 ...
                                                                 eps_scale = 0.,
                                                                 seed = seed_dist,
                                                                 scale_def = None, # could be None, std, cov, off
                                                                 nsamples = 50_000)
    ...
    while metric_threshold_number < len(metric_thresholds) and iteration < max_iterations:
        ...
        dist_2 = MixtureDistributions.MultiNormalFromMixtureGaussian(ncomp = ncomp,
                                                                     ...
                                                                     eps_scale = eps_std,
                                                                     seed = seed_dist,
                                                                     scale_def = scale_def, # could be None, std, cov, off
                                                                     nsamples = 50_000)
