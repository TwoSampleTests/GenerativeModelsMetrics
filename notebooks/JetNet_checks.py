...
# Compute values of FPD for different values of samples/nmax
for max_samples in [25_000,30_000,35_000,40_000,45_000,50_000]:
    fpd = JMetrics.fpd(real_features = s1[:max_samples],
                       ...
                       normalise = True,
                       seed = 42)
    print(f"nmax = {max_samples} -> fpd = {fpd}")
    
# Compute values of KPD for different values of batch_size
for batch_size in [100,1_000,1_500,2_000,3_000,5_000]:
    kpd = JMetrics.kpd(real_features = s1,
                       gen_features = s2,
                       num_batches = 10,
